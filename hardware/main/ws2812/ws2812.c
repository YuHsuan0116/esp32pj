...
esp_err_t del_ws2812_encoder(rmt_encoder_t* encoder) {
    ws2812_encoder_t* ws2812_encoder = __containerof(encoder, ws2812_encoder_t, base);
...
esp_err_t ws2812_encoder_reset(rmt_encoder_t* encoder) {
    ws2812_encoder_t* ws2812_encoder = __containerof(encoder, ws2812_encoder_t, base);
...
esp_err_t new_ws2812_encoder(rmt_encoder_handle_t* ret_encoder) {
    ...
    ws2812_encoder = rmt_alloc_encoder_mem(sizeof(ws2812_encoder_t));
    ws2812_encoder->base.encode = encode_ws2812;
    ws2812_encoder->base.del = del_ws2812_encoder;
...
esp_err_t new_ws2812(int _gpio_num, int _LED_NUM, ws2812_handle_t* ret_ws2812) {
    ret_ws2812->channel = NULL;
    new_ws2812_encoder(&(ret_ws2812->encoder));
    ...
    ESP_RETURN_ON_ERROR(rmt_new_tx_channel(&tx_chan_config, &ret_ws2812->channel), TAG, "new_tx_channel failed");
    ESP_RETURN_ON_ERROR(rmt_enable(ret_ws2812->channel), TAG, "enable failed");
    ret_ws2812->LED_NUM = _LED_NUM;
    return ESP_OK;
}
...
esp_err_t display(uint8_t data[][3], ws2812_handle_t ws2812) {
    rmt_transmit(ws2812.channel, ws2812.encoder, data, ws2812.LED_NUM * 3, &tx_config);
